...
role = "student"  # The Proxmox role to revoke
    ...
    for row in reader:
        ...
        for vmid in vm_ids:
            for host in proxmox_hosts:
                print(f"   🧹 Removing VM {vmid} on {host} → {pve_user}")
                cmd = [
                    "ssh", "-o", "StrictHostKeyChecking=no",
                    f"{ssh_user}@{host}",
                    f"pveum acl delete /vms/{vmid} -user '{pve_user}' --roles {role}"
                ]
